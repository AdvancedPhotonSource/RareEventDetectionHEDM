...
if __name__ == "__main__":
    ...
    parser.add_argument('-testing_scan',  type=str, required=True, help='testing scan(s), it is a folder of all scans if patch mode is selected, it is a single file if file-based mode is selected')
    parser.add_argument('-testing_scan_dark',  type=str, default="dark", help='input dark file for the testing scan, it will be required if the streaming version is enabled')

    parser.add_argument('-output_csv',     type=str, required=True, help='result csv file')
    ...
    Testing_run = Run(args)

    Testing_run.testing_process()
